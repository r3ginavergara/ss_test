...
if($status){
    $updatequery = "UPDATE categories SET category='$update_category', description='$description',status='$status',image='$image' WHERE id='$update_id'";
    $updateconfirm = mysqli_query($conn,$updatequery);

    setFlashMessage('success', 'YAY! Account Updated!');
...
<body>
	...
	<div class="card-body ">
        ...
        <div class="container-fluid">
			<div>
		<a class="btn btn-outline-primary" href="category.php" <span class="fa fa-trash text-danger"></span> Go back</a>
                   
                    </div>
                        <div class="modal-body border p-5 mx-auto  w-50">
                        <?php
                        	$qry = $conn->query("SELECT * FROM categories WHERE id = '$request_id'");
					       ...
					       ?>
                            ...
                            <input type="hidden" value="<?=$id?>" name="id">
                                ...
                                <center> <img src="<?php echo '../../images/'.$image?>"  width="260px" height="260px" target="_self"><br>
                                <h4></h4>
                                <h3><?php echo $name?></h3>
                                </center><hr>
                                <div class="form-group">
                            <label for="Category" style="color:#0A5C36;">Category:</label>
                            <input type="text" class="form-control" id="category" name="category" value="<?=$name?>"required>
                            </div>
                            <div class="form-group">
                            ...
                            <label for="status" style="color:#0A5C36;">Status:</label>
                                <select name="status" class="form-control" id="status" required>
                                    <option value="<?=$status?>"><?php if($status==1){echo "Active";}else{echo "Inactive";}?></option> 
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <?php
                   $req = $_GET['id'];
                   $qrys = $conn->query("SELECT * FROM categories WHERE id = '$req'");
